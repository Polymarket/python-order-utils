...
class MarketOrderBuilder(BaseBuilder):
    ...
    def build_market_order_and_signature(
        self,
        ...
        """
        return MarketOrderAndSignature(
            mkt_order,
            signature,
            "market",
    ...
    def create_market_order(self, data: MarketOrderData):
        ...
        return MarketOrderAndSignature(
            ...
            signature=sig,
            orderType="market",
            minAmountReceived=data.min_amount_received,
    ...
    def _normalize(self, mkt_order: MarketOrder):
        return MarketOrder(
            ...
            makerAssetID=mkt_order["makerAssetID"]
            if mkt_order["makerAssetID"] >= 0
            else 0,
            takerAsset=mkt_order["takerAsset"],
            takerAssetID=mkt_order["takerAssetID"]
            if mkt_order["takerAssetID"] >= 0
            else 0,
    ...
    def _validate_inputs(self, data: MarketOrderData) -> bool:
        return not (
            ...
            or (data.maker_asset_id is None and data.taker_asset_id is None)
            or normalize_address(data.exchange_address) != self.contract_address
        )
