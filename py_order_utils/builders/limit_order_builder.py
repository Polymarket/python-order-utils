...
class LimitOrderBuilder(BaseBuilder):
    ...
    def build_limit_order(self, data: LimitOrderData) -> LimitOrder:
        ...
        if data.maker_asset_id is not None:
            ...
            maker_asset_data = self.erc20_facade.transfer_from(
                data.maker_address, data.taker_address, data.maker_amount
            )
        ...
        if data.taker_asset_id is not None:
            ...
            taker_asset_data = self.erc20_facade.transfer_from(
                data.taker_address, data.maker_address, data.taker_amount
            )

        predicate = (
            data.predicate
            if (data.expiry is None and data.nonce is None)
            else self.lop_facade.lop_and(
        ...
        get_maker_amount = self.lop_facade.get_maker_amount_data(
            data.maker_amount, data.taker_amount
        )
        get_taker_amount = self.lop_facade.get_taker_amount_data(
            data.maker_amount, data.taker_amount
        )
    ...
    def build_limit_order_and_signature(self, limit_order: LimitOrder, signature: str):
        return LimitOrderAndSignature(
            order=limit_order, signature=signature, orderType="limit"
        )
    ...
    def _validate_inputs(self, data: LimitOrderData) -> bool:
        return not (
            ...
            data.exchange_address is None
            or data.maker_asset_address is None
            or data.taker_asset_address is None
            or data.maker_address is None
